...
from askchart.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN, DEFAULT_VIDEO_TOKEN, DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, MAX_IMAGE_LENGTH, \
    MAX_VIDEO_LENGTH
from torch.utils.data import Dataset
from askchart.train.askchart_trainer import AskChartTrainer

from askchart import conversation as conversation_lib
from askchart.model import *
from askchart.mm_utils import tokenizer_image_token

from PIL import Image
from askchart.utils import order_pick_k
import ipdb
...
def unlock_vit(training_args, model_args, image_tower):
    lr_of_vit = training_args.image_tower_lr if training_args.image_tower_lr is not None and training_args.image_tower_lr != 0 else training_args.learning_rate
    rank0_print(
        f'Tune the image tower! LR for ViT is {lr_of_vit}.')
    if model_args.tune_vit_from_layer != -1:
        rank0_print(
            f'Tune the image tower from layer {model_args.tune_vit_from_layer}!')
    for n, p in image_tower.named_parameters():
        if getattr(training_args, 'tune_vit_posemb_only', False):
            ...
            if 'image_tower.vision_model.encoder.layers.' in n:
                layer_id = int(
                    n.split('image_tower.vision_model.encoder.layers.')[-1].split('.')[0])
...
class ModelArguments: 
    ...
    tune_image_tower: bool = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora_bias: str = "none"
    image_tower_lr: Optional[float] = None
...
def get_image_tower_state_maybe_zero_3(named_params, keys_to_match=['']):
    to_return = {k: t for k, t in named_params if any(key_match in k for key_match in keys_to_match)}
    to_return = {k: maybe_zero_3(v, ignore_status=True).cpu() for k, v in to_return.items()}
    return to_return
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        # save image tower
        if getattr(trainer.args, "tune_image_tower", False):
            rank0_print(f'get attr tune_image_tower')
            if trainer.deepspeed:
                torch.cuda.synchronize()
            trainer.model.get_image_tower().image_processor.save_pretrained(
                os.path.join(output_dir, 'image_tower'))
            rank0_print(f'Save image tower to {os.path.join(output_dir, "image_tower")}')
            trainer.model.get_image_tower().image_tower.vision_model.config.save_pretrained(
                os.path.join(output_dir, 'image_tower'))
            weight_to_save = get_image_tower_state_maybe_zero_3(trainer.model.get_image_tower().image_tower.named_parameters())
            if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
                torch.save(weight_to_save, os.path.join(output_dir, 'image_tower/pytorch_model.bin'))
        else:
            rank0_print(f'not get attr tune_image_tower')
        return
    
    if getattr(trainer.args, "tune_image_tower", False) or getattr(trainer.args, "tune_entire_model", False):
        if trainer.deepspeed:
            torch.cuda.synchronize()
        if getattr(trainer.model.get_image_tower().image_processor, 'save_pretrained', False):
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            torch.save(weight_to_save, os.path.join(
                output_dir, 'image_tower/pytorch_model.bin'))
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if 'image' in sources[0] and 'video' not in sources[0]:
                ...
                # print(image[0])
                if self.data_args.image_aspect_ratio == 'pad': # process image
...
def train():
    ...
    parser = transformers.HfArgumentParser(
        (ModelArguments, DataArguments, TrainingArguments)) # load arguments
    ...
    if model_args.image_tower is not None or model_args.video_tower is not None:
        if not model_args.moe_enable:
            if 'phi' in model_args.model_name_or_path.lower():
                model = AskChartPhiForCausalLM.from_pretrained(
                ...
                model = AskChartLlamaForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    # attn_implementation="flash_attention_2",
                    # torch_dtype=torch.bfloat16,
            ...
            if 'phi' in model_args.model_name_or_path.lower():
                model = MoEAskChartPhiForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    # local_files_only=True,
                    # attn_implementation="flash_attention_2",
                ...
                model = MoEAskChartLlamaForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation="flash_attention_2",
                    torch_dtype=torch.bfloat16,
    ...
    if model_args.image_tower is not None or model_args.video_tower is not None:
        ...
        if model_args.image_tower is not None:
            ...
            # set gradient checkpointing for image tower, added by sootung
            if training_args.gradient_checkpointing:
                image_tower.image_tower.gradient_checkpointing_enable(
                    gradient_checkpointing_kwargs={"use_reentrant": False}
                )
                if hasattr(image_tower.image_tower, "enable_input_require_grads"):
                    image_tower.image_tower.enable_input_require_grads()
                else:
                    def make_inputs_require_grad(module, input, output):
                        output.requires_grad_(True)
                    image_tower.image_tower.get_input_embeddings().register_forward_hook(make_inputs_require_grad)

            data_args.image_processor = image_tower.image_processor
            data_args.is_multimodal = True
            # print(f'data_args.image_processor: {data_args.image_processor}')
            # ipdb.set_trace()
    ...
    # =============================================================================================================
        ...
        if training_args.freeze_mm_mlp_adapter:
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = False
        # tune image tower
        model.config.tune_image_tower = training_args.tune_image_tower = model_args.tune_image_tower
        ...
        if model_args.tune_entire_model:
            ...
            if training_args.lora_enable: # TODO: check the code, adapt to tune entire model. check if vision tower is saved.
                unlock_vit(training_args, model_args, image_tower)
            else:
                model.requires_grad_(True)
                unlock_vit(training_args, model_args, image_tower)
        elif model_args.tune_image_tower:
            rank0_print(f'Tune image tower!')
            unlock_vit(training_args, model_args, image_tower)
    ...
    for name, param in model.named_parameters():
        ...
        if param.requires_grad:
            ...
            rank0_print('trainable modules: ',name)
    rank0_print(f"Total parameters: {total_params}, trainable parameters: {trainable_params}")
    rank0_print(model)
    # sys.exit()

    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    trainer = AskChartTrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
